...
def _compute_weight_gradient(grad, input_activation, causal_set, spike_time, A, B, W, layer_param):
    ...
    grad_w = torch.where(causal_set==1, grad_w_real, layer_param.zero)
...
def _compute_input_gradient_real(grad, input_activation, weight, A, B, W, decay_params):
    e_K_tp = torch.exp(decay_params['rate']*input_activation)
    d = weight * e_K_tp[None, :] * (decay_params['rate'] * (input_activation[None, :] - (B / A)[:, None]) + W[:, None] + 1) / (A * (1.0 + W))[:, None]
    d = torch.clip(d, -tempcoder.clip_derivative, tempcoder.clip_derivative)
    return torch.clip(grad[:, None]*d, -tempcoder.clip_derivative, tempcoder.clip_derivative)
...
def _compute_input_gradient(grad, input_activation, causal_set, spike_time, weight, A, B, W, layer_param):
    ...
    return torch.sum(torch.where(check, grad_x_real, layer_param.zero), axis=0)
...
class LinearFunction(torch.autograd.Function):
    ...
    def forward(ctx, input, weight, pulse, layer_param):
        input_activation = torch.cat([input, pulse])
...
class CrossEntropyLoss(torch.autograd.Function):
    ...
    def forward(ctx, x, target, penalty_output_spike_time=0):
        ...
        min_value = torch.min(x)
        loss = _torch_cross_entropy_loss_with_kEps(min_value-x, target)
        loss += penalty_output_spike_time * torch.sum(x*x)
...
class LayerParam(nn.Module):
    def __init__(self, kNoSpike, decay_rate, threshold, penalty_no_spike, pulse_init_multiplier, nopulse_init_multiplier, input_range, dtype = torch.double) -> None:
        super(LayerParam, self).__init__()
        self.kNoSpike = torch.tensor(kNoSpike,dtype=dtype)
        self.decay_rate = torch.tensor(decay_rate,dtype=dtype)
        self.decay_rate_inverse = 1/self.decay_rate
        self.decay_params = {'rate': self.decay_rate, 'rate_inverse': self.decay_rate_inverse}
        self.threshold = torch.tensor(threshold,dtype=dtype)
        self.kMinLambertArg = torch.tensor(-1.0 / np.e, dtype=dtype)
        self.kMaxLambertArg = torch.tensor(1.7976131e+308, dtype=dtype)
        self.zero = torch.tensor(0.0, dtype=dtype)
        self.penalty_no_spike = torch.tensor(penalty_no_spike, dtype=dtype)
